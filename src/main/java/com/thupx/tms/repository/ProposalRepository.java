...
public interface ProposalRepository extends JpaRepository<Proposal, Long> {
	...
	@Query(value = "SELECT * FROM proposal as p, jhi_user as u, hospital_department as h WHERE p.user_extra_user_id = u.id and p.hospital_department_id = h.id and (UPPER(p.content_proposal) LIKE UPPER(CONCAT('%', TRIM(:search), '%')) or UPPER(p.current_progress_name) LIKE UPPER(CONCAT('%', TRIM(:search), '%')) or UPPER(p.note) LIKE UPPER(CONCAT('%', TRIM(:search), '%')) or (CASE WHEN isnumeric(TRIM(:search)) = true THEN p.id = CAST(TRIM(:search) AS INT) END) or UPPER(u.first_name) LIKE UPPER(CONCAT('%', TRIM(:search), '%')) or UPPER(u.last_name) LIKE UPPER(CONCAT('%', TRIM(:search), '%')) or UPPER(h.hospital_department_name) LIKE UPPER(CONCAT('%', TRIM(:search), '%')))", nativeQuery = true)
